...
def test():
    ...
    try:
        ...
        result = execute_query(connection,
            "SELECT * FROM customers WHERE customer_id = %s", parameters,
            fetch="all"
        )
        if result:
            print(result)

    except InvalidQueryOptionsError as e:
        print(f"Caught error: {e}")

    except FetchUpdateConflictError as e:
            print(f"Caught error: {e}")

    except psycopg.errors.UndefinedTable as e:
        print(f"Caught error. Table doesn't exist: {e}")

    except psycopg.errors.ForeignKeyViolation as e:
        print(f"Invalid customer or barber: {e}")

    except Exception as e:
        print(f"Caught unexpected database error: {e}")
