...
def build_team_sheet(name):
    ...
    # ---------- DATA VALIDATION ----------
    # Player dropdown (source: CALCULATIONS column A — hidden sheet)
    ...
    # Captain dropdown (source: this sheet's own C10:C14)
...
market.column_dimensions["E"].width = 18

title(market, "PLAYER MARKET", 1, 2, 5)

market.merge_cells("B2:E2")
market["B2"] = "Prices  •  Ownership  •  Gameweek Points"
...
for c, h in enumerate(["Player", "Price", "Ownership %", "Points"], start=2):
    market.cell(4, c, h)
...
for i, idx in enumerate(range(2, 23)):
    ...
    market.cell(r, 5, f"=CALCULATIONS!D{idx}")
...
awards = [
    ("🏆 Highest Scorer",
     '=INDEX(C5:C25,MATCH(MAX(D5:D25),D5:D25,0))'),
    ("📈 Most Improved Player", "Coming after Gameweek 2"),
    ("🔥 Most Selected Player",
     "=INDEX('PLAYER MARKET'!B5:B25,MATCH(MAX('PLAYER MARKET'!D5:D25),'PLAYER MARKET'!D5:D25